...
class Upsampler4(nn.Module):
    def __init__(self, input_dim, output_dim, network_capacity=32):
        super(Upsampler4, self).__init__()
        ...
        self.up4 = ExtendedUp(dims[4], dims[4])
        self.up5 = ExtendedUp(dims[4], dims[4])
...
class Upsampler5(nn.Module): # from 256 -> 512
    def __init__(self, input_dim, output_dim, network_capacity=32):
        super(Upsampler5, self).__init__()
        ...
        self.down1 = ExtendedDown(dims[0], dims[1])
        self.down2 = ExtendedDown(dims[1], dims[2])
        self.down3 = ExtendedDown(dims[2], dims[2])
        ...
        # Extended Upsampling with skip connections
        self.up1 = ExtendedUp(dims[3] + dims[2], dims[4])
        self.up2 = ExtendedUp(dims[4] + dims[2], dims[4])
        self.up3 = ExtendedUp(dims[4] + dims[1], dims[4])
        self.up4 = ExtendedUp(dims[4] + dims[0], dims[4])
        # self.up5 = ExtendedUp(dims[4], dims[4])
    ...
    def forward(self, x):
        ...
        x5 = self.bridge(x4)      # 32x32
        y = self.up1(x5, x4)      # 32x32
        y = self.up2(y, x3)       # 64x64
        y = self.up3(y, x2)       # 128x128
        y = self.up4(y, x1)       # 256x256
        # y = self.up5(y)           # 512x512
        y = self.outc(y)
        return y


class ExtendedUpWithTranspose(nn.Module):
    """Extended upsampling using ConvTranspose2d and more convolutions"""
...
if __name__ == "__main__":
    ...
    # check the upsample4
    upsampler4 = Upsampler4(32, 3).cuda()
    y = upsampler4(x)
